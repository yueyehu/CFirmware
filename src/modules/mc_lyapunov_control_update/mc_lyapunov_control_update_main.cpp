...
/**
 * @file MC_LYAPUNOV_UP_control_main_update.cpp
 ...
 * @ingroup apps
 */
extern "C" __EXPORT int MC_LYAPUNOV_UP_control_main(int argc, char *argv[]);

class MulticopterLyapunovControlUpdate : public control::SuperBlock
{
public:
    /**
     * Constructor
     */
    MulticopterLyapunovControlUpdate();
...
MulticopterLyapunovControlUpdate	*g_control;
}

MulticopterLyapunovControlUpdate::MulticopterLyapunovControlUpdate() :
    SuperBlock(NULL, "MPC"),
    ...
    _in_landing(false)
{
    ...
    _params_handles.Ix		= param_find("MC_LYAPUNOV_UP_IX");
    _params_handles.Iy		= param_find("MC_LYAPUNOV_UP_IY");
    _params_handles.Iz		= param_find("MC_LYAPUNOV_UP_IZ");
    _params_handles.m		= param_find("MC_LYAPUNOV_UP_M");
    _params_handles.k1		= param_find("MC_LYAPUNOV_UP_K1");
    _params_handles.k2		= param_find("MC_LYAPUNOV_UP_K2");
    _params_handles.k3		= param_find("MC_LYAPUNOV_UP_K3");
    _params_handles.k4		= param_find("MC_LYAPUNOV_UP_K4");
    _params_handles.k5		= param_find("MC_LYAPUNOV_UP_K5");
    _params_handles.k6		= param_find("MC_LYAPUNOV_UP_K6");
    _params_handles.k7		= param_find("MC_LYAPUNOV_UP_K7");
    _params_handles.k8		= param_find("MC_LYAPUNOV_UP_K8");
    _params_handles.k9		= param_find("MC_LYAPUNOV_UP_K9");
...
int
MulticopterLyapunovControlUpdate::parameters_update(bool force)
{
    bool updated;
...
void
MulticopterLyapunovControlUpdate::poll_subscriptions()
{
    bool updated;
...
void
MulticopterLyapunovControlUpdate::task_main_trampoline(int argc, char *argv[])
{
    lyapunov_control::g_control->task_main();
}

void
MulticopterLyapunovControlUpdate::update_ref()
{
...
float
MulticopterLyapunovControlUpdate::limitMinMax(float input, float _min, float _max){
    if(input < _min){
        return _min;
    }
    else if(input > _max){
...
void
MulticopterLyapunovControlUpdate::control_auto(float dt)
{
...
void
MulticopterLyapunovControlUpdate::task_main()
{
    ...
    parameters_update(true);
    //mavlink_log_info(_mavlink_fd, "[MC_LYAPUNOV_UP]param:%8.4f,%8.4f,%8.4f",(double)_params.Ix,(double)_params.Iy,(double)_params.Iz);
	...
	math::Matrix<3,3> I;
	I.identity();
	I(0,0)=_params.Ix;I(1,1)=_params.Iy;I(2,2)=_params.Iz;
	
	math::Matrix<3, 3> J_inv;
	
    math::Vector<3> tau;
	tau.zero();
	
	math::Matrix<3,3> K_eta;
	K_eta.identity();
	K_eta(0,0) = _params.k1;K_eta(1,1) = _params.k3;K_eta(2,2) = _params.k5;
	
	math::Matrix<3,3> K_peta;
	K_peta.identity();
	K_peta(0,0) = _params.k2;K_peta(1,1) = _params.k4;K_peta(2,2) = _params.k6;
	
	math::Vector<3> e_eta;
	e_eta.zero();
	math::Vector<3> e_peta;
	e_peta.zero();
	
	math::Matrix<3,3> K_xi;
	K_xi.identity();
	K_xi(0,0) = _params.k7;K_xi(1,1) = _params.k9;K_xi(2,2) = _params.k11;
	
	math::Matrix<3,3> K_pxi;
	K_pxi.identity();
	K_pxi(0,0) = _params.k8;K_pxi(1,1) = _params.k10;K_pxi(2,2) = _params.k12;
    ...
    while (!_task_should_exit) {
        ...
        if (_ctrl_state.timestamp > 0){
            ...
            mavlink_log_info(_mavlink_fd, "[MC_LYAPUNOV_UP]degree:%8.4f,%8.4f",(double)psi,(double)dpsi);
            ...
            mavlink_log_info(_mavlink_fd, "[MC_LYAPUNOV_UP]log:attitude error");
            continue;
        }
		J_inv(0,0) = (_params.Iy*_params.Iz+(_params.Ix-_params.Iz)*_params.Iy*sinf(theta)*sinf(theta)-(_params.Iy-_params.Iz)*_params.Ix*(float)(sinf(phi)*sinf(phi)*sinf(theta)*sinf(theta)))/\
                (_params.Ix*_params.Iy*_params.Iy*cosf(theta)*cosf(theta));
        J_inv(1,1) = (_params.Iy*sinf(phi)*sinf(phi)+_params.Iz*cosf(phi)*cosf(phi))/(_params.Iy*_params.Iz);
		J_inv(2,2) = (_params.Iy*cosf(phi)*cosf(phi)+_params.Iz*sinf(phi)*sinf(phi))/(_params.Iy*_params.Iz*cosf(theta)*cosf(theta));
		J_inv(0,1)=J_inv(1,0)=-(_params.Iy-_params.Iz)*cosf(phi)*sinf(phi)*sin(theta)/(_params.Iy*_params.Iz*cosf(theta));
		J_inv(0,2)=J_inv(2,0)=(_params.Iy*cosf(phi)*cosf(phi)+_params.Iz*sinf(phi)*sinf(phi))*sinf(theta)/(_params.Iy*_params.Iz*cosf(theta)*cosf(theta));
		J_inv(1,2)=J_inv(2,1)=-(_params.Iy-_params.Iz)*cosf(phi)*sinf(phi)/(_params.Iy*_params.Iz*cosf(theta));
		g1 = J_inv(0,0);
        g2 = J_inv(1,1);
        g3 = J_inv(2,2);
        ...
        //mavlink_log_info(_mavlink_fd, "[MC_LYAPUNOV_UP]log:%8.4f,%8.4f,%8.4f",(double)g1,(double)g2,(double)g3);

        if (_control_mode.flag_control_auto_enabled) {
            PX4_WARN("[MC_LYAPUNOV_UP]log:auto mode is set");
            control_auto(dt);
            ...
            if (!_control_mode.flag_control_manual_enabled && _pos_sp_triplet.current.valid
				...
				tau = I*((K_eta*J_inv*I*K_eta-J_inv*I)*e_eta-(K_eta+K_peta)*J_inv*I*e_peta);
...
int
MulticopterLyapunovControlUpdate::start()
{
    ASSERT(_control_task == -1);

    /* start the task */
    _control_task = px4_task_spawn_cmd("MC_LYAPUNOV_UP_control",
                       SCHED_DEFAULT,
                       SCHED_PRIORITY_MAX - 5,
                       1900,
                       (px4_main_t)&MulticopterLyapunovControlUpdate::task_main_trampoline,
                       nullptr);
...
int
MC_LYAPUNOV_UP_control_update_main(int argc, char *argv[])
{
    if (argc < 2) {
        warnx("usage: MC_LYAPUNOV_UP_control_update {start|stop|status}");
        return 1;
    }
    ...
    if (!strcmp(argv[1], "start")) {
        ...
        lyapunov_control::g_control = new MulticopterLyapunovControlUpdate;
